...
// /v1/models and /code/v1/models are intentionally placed under RequireFlexibleAuth
// (which accepts both x-api-key and Bearer) so that clients using the native Anthropic
// auth header (x-api-key) can also reach the model list endpoint.
...
impl RouterBuilder {
    ...
    fn route_claude_web_endpoints(mut self) -> Self {
        let router = Router::new()
            ...
            // /v1/models is here (not in the OAI group) so that native Anthropic clients
            // sending x-api-key are also accepted by RequireFlexibleAuth.
    ...
    fn route_claude_code_endpoints(mut self) -> Self {
        let router = Router::new()
            ...
            // /code/v1/models is here so that native Anthropic clients sending x-api-key
            // are accepted alongside Bearer-token clients, via RequireFlexibleAuth.
    ...
    fn route_claude_web_oai_endpoints(mut self) -> Self {
        let router = Router::new()
            ...
            // /v1/models is registered in route_claude_web_endpoints under RequireFlexibleAuth
            // so it also accepts x-api-key (native Anthropic auth). No duplicate here.
            .layer(
                ServiceBuilder::new()
    ...
    fn route_claude_code_oai_endpoints(mut self) -> Self {
        let router = Router::new()
            ...
            // /code/v1/models is registered in route_claude_code_endpoints under RequireFlexibleAuth
            // so it also accepts x-api-key (native Anthropic auth). No duplicate here.
            .layer(
                ServiceBuilder::new()
                    .layer(from_extractor::<RequireBearerAuth>())
